                            ...
                            Date toDate = toDateString.matches("\\d{2}/\\d{2}/\\d{4}") ? sdf.parse(toDateString) : null;
                             ...
                             dateFilter = new RowFilter<DefaultTableModel, Integer>() {
                                public boolean include(Entry<? extends DefaultTableModel, ? extends Integer> entry) {
                                    try {
                                        String dateString = (String) entry.getValue(entry.getModel().getColumnCount() - 1);
            ...
            public void keyPressed(KeyEvent e) {
                if (e.getKeyCode() == KeyEvent.VK_ENTER) {
                    ...
                    if (!nhanVien.isEmpty()) {
                        ...
                        if (dateFilter != null) {
                            if (roomFilter != null) {
                                rowSorter.setRowFilter(RowFilter.andFilter(Arrays.asList(dateFilter, roomFilter, staffFilter)));
                            } else {
                                rowSorter.setRowFilter(RowFilter.andFilter(Arrays.asList(dateFilter, staffFilter)));
                            }
                        } else {
